...
import { api, bandColor, LANGS, SAMPLES } from '../lib/api'
      ...
      <div style={{ display: 'grid', gridTemplateColumns: result ? 'minmax(0, 1fr) minmax(0, 1fr)' : 'minmax(0, 1fr)', gap: 16 }} className="ai-grid">
        <Reveal className="glass" style={{ padding: 18, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {[['AI-style', SAMPLES.ai_python], ['Human', SAMPLES.clean_python]].map(([l, sn]) => (
              <button key={l} onClick={() => { setCode(sn); setLang('Python') }} className="tag" style={{ cursor: 'pointer' }}>{l}</button>
            ))}
          </div>
